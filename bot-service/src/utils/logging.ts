...
import { isHttpError } from '@/resources/exceptions/HttpException';
...
const handleRequestError = (error: unknown, response: Response, logOnly?: true) => {
    ...
    return response.status(parsedError.statusCode).send({ message: parsedError.message });
};
...
const parseUnknownError = (error: unknown): ParsedError => {
    ...
    if (isAxiosError(error)) {
        ...
        parsedError['statusCode'] = error.response?.status ?? error.status ?? INTERNAL_SERVER_ERROR;
